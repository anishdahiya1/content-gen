    ...
    <main className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 py-12 px-4">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mb-12 animate-fade-in">
          ...
          <h1 className="text-4xl font-bold text-white mb-2">📤 Publish to Platforms</h1>
          <p className="text-slate-400">One-click publishing to all your social networks</p>
        </div>

        {/* Platform Grid */}
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
          {platforms.map((platform) => (
            <div key={platform.id} className={`card-hover border border-slate-700 p-6 cursor-pointer group hover:scale-105 transition-all bg-gradient-to-br ${platform.color} opacity-20 hover:opacity-30`}>
              <div className="text-5xl mb-3">{platform.icon}</div>
              <h3 className="font-semibold text-lg text-white group-hover:text-cyan-300 transition-colors">{platform.name}</h3>
              <p className="text-slate-300 text-sm mt-2">Ready to publish</p>
        ...
        {/* Publish Summary */}
        <div className="card">
          <h2 className="text-xl font-semibold text-white mb-6">📋 Publishing Checklist</h2>
          <div className="space-y-3">
            {[
              ...
              <div key={i} className={`flex items-center gap-3 p-3 rounded-lg ${item.done ? 'bg-green-900/20 border border-green-700' : 'bg-yellow-900/20 border border-yellow-700'}`}>
                <span className={item.done ? 'text-green-400' : 'text-yellow-400'}>{item.done ? '✓' : '○'}</span>
